...
namespace Be.Stateless.BizTalk.ServiceModel.Channels
{
	...
	public class XmlMessage : IXmlSerializable
	{
		...
		/// <summary>
		/// This method is reserved and should not be used. When implementing the <see cref="IXmlSerializable"/> interface, you
		/// should return <c>null</c> from this method, and instead, if specifying a custom schema is required, apply the <see
		...
		/// root element surrounding the response <see cref="XmlMessage"/> will be provided by WCF.
		/// </remarks>
		...
		public virtual string Action => OperationContext.Current.IncomingMessageHeaders.Action;

		/// <summary>
		/// Gets the SOAP version of the message.
		/// </summary>
		[SuppressMessage("ReSharper", "VirtualMemberNeverOverridden.Global", Justification = "Public API.")]
		public virtual MessageVersion Version => OperationContext.Current.IncomingMessageVersion;
		...
		{
			var xmlReader = XmlReader.Create(new StringReader(RawXmlBody), new() { CloseInput = true });
