...
namespace Be.Stateless.BizTalk.ServiceModel
{
	...
	public abstract class ServiceRelay
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="ServiceRelay"/> class using the configuration information specified in
		/// the application configuration file by <paramref name="endpointConfigurationName"/>.
		/// </summary>
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="ServiceRelay"/> class using the specified <paramref name="binding"/> and
		/// target <paramref name="endpointAddress"/>.
		/// </summary>
		/// <param name="binding">
		...
		/// <summary>
		/// Relays an <see cref="XmlMessage"/>-based request and returns the correlated <see cref="XmlMessage"/>-based response.
		/// </summary>
		...
		/// <summary>
		/// Converts and relays an <see cref="XmlMessage"/>-based request and, converts and returns the correlated <see
		...
		/// <remarks>
		/// <para>
		/// If a timeout is passed while calling the function then that value is used. If the <see cref="Binding.SendTimeout"/>
		/// is set on the binding, then the value on the binding is used if no timeout is specified while calling the function.
		/// </para>
		/// <para>
		/// The <see cref="ChannelBase.DefaultSendTimeout"/> is used if no timeout is specified on either the binding or while
		...
		private TResponse RelayRequest<TRequest, TResponse>(TRequest request, Func<ClientRelay, TimeSpan> getTimeout, IXmlMessageConverter converter)
			...
			where TResponse : XmlMessage, new()
		{
			if (_logger.IsInfoEnabled) _logger.InfoFormat("Relaying synchronous request {0}.", request);
			...
			try
			{
				...
				clientRelay.Close();
				if (_logger.IsInfoEnabled) _logger.InfoFormat("Relaying synchronous response {0} succeeded.", response);
				return response;
			}
			catch (Exception exception)
			{
				if (_logger.IsWarnEnabled) _logger.Warn($"Relaying synchronous response {typeof(TResponse).Name} failed.", exception);
				clientRelay?.Abort();
				throw;
			}
		}
		...
		/// <summary>
		/// Begins an asynchronous operation to convert and relay an <see cref="XmlMessage"/>-based request to the reply side of
		...
		private IAsyncResult BeginRelayRequest<TRequest>(
			...
			where TRequest : XmlMessage
		{
			if (_logger.IsInfoEnabled) _logger.InfoFormat("Beginning relaying asynchronous request {0}.", request);
			ClientRelay clientRelay = null;
			try
			{
				clientRelay = _clientRelayFactory();
			...
			{
				if (_logger.IsWarnEnabled) _logger.Warn($"Relaying asynchronous request {typeof(TRequest).Name} failed.", exception);
				clientRelay?.Abort();
				throw;
			}
		}
			...
			where TResponse : XmlMessage, new()
		{
			...
			try
			{
				...
				clientRelay.Close();
				if (_logger.IsInfoEnabled) _logger.InfoFormat("Relaying asynchronous response {0} succeeded.", response);
				return response;
			}
			catch (Exception exception)
			{
				if (_logger.IsWarnEnabled) _logger.Warn($"Relaying asynchronous response {typeof(TResponse).Name} failed.", exception);
				clientRelay?.Abort();
				throw;
			}
		}
