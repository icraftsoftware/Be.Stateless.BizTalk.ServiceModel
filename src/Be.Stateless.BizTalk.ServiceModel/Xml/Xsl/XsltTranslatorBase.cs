...
{
	/// <summary>
	/// Base strategy that translates <see cref="XmlMessage"/>s back and forth to native generic WCF <see cref="Message"/>s by
	/// applying an <see cref="XslCompiledTransform"/> to their payloads.
	/// </summary>
	public abstract class XsltTranslatorBase : IXmlMessageConverter
	{
		...
		{
			if (xmlResponse == null) throw new ArgumentNullException(nameof(xmlResponse));
		...
		/// </summary>
		/// <param name="version">
		/// A <see cref="MessageVersion"/> object that specifies the SOAP version to use for the message. It defaults to the
		/// version of the incoming <see cref="XmlMessage"/> request; see <see cref="XmlMessage.Version"/>.
		/// </param>
		/// <param name="action">
		...
		/// </param>
		/// <param name="reader">
		/// The <see cref="XmlReader"/> object to be used for reading the SOAP message. It defaults to the output of the <see
		/// cref="RequestXslt"/> that has been applied to the body of the request <see cref="XmlMessage"/>.
		/// </param>
		/// <returns>
		/// A <see cref="Message"/> object for the message created.
		/// </returns>
		[SuppressMessage("ReSharper", "VirtualMemberNeverOverridden.Global", Justification = "Public API.")]
		protected virtual System.ServiceModel.Channels.Message CreateTranslatedRequestMessage(MessageVersion version, string action, XmlReader reader)
		{
			return System.ServiceModel.Channels.Message.CreateMessage(version, action, reader);
		}
