...
{
	/// <summary>
	/// Activates the <see cref="SimpleServiceHost{TService,TChannel}"/>, i.e. the in-process host with a single WCF service
	/// endpoint meant to facilitate the NUnit-based unit testing of WCF service contracts.
	/// </summary>
	/// <remarks>
	/// <see cref="SimpleServiceHostActivatorAttribute"/> ensures that the <see cref="SimpleServiceHost{TService,TChannel}"/>
	/// with a single WCF service endpoint listening and started before any NUnit-test depending on it runs. Making use of this
	/// attribute therefore assumes and requires that unit tests are written on the basis of the NUnit testing framework.
	/// </remarks>
	...
	public class SimpleServiceHostActivatorAttribute : TestActionAttribute
	{
		...
		/// cref="SimpleServiceHost{TService,TChannel}"/> with a single endpoint listening at <paramref name="address"/> over
		/// <paramref name="binding"/> is started before any NUnit-test depending on it runs.
		/// </summary>
		/// <param name="service">
		...
		/// The interval of time in seconds provided for a write operation to complete before the transport raises an exception.
		/// </param>
		/// <remarks>
		/// Don't forget to <see href="http://msdn.microsoft.com/en-us/library/ms733768.aspx">configure HTTP</see>:
		/// <code><![CDATA[netsh http add urlacl url=http://+:8001/calculator user=$env:USERDOMAIN\$env:USERNAME]]>!</code>
